 ...
 *   2. git rev-parse --show-toplevel → projectDir/.kstack/
...
export function resolveConfig(
  ...
  if (env.BROWSE_STATE_FILE) {
    ...
    projectDir = path.dirname(stateDir); // parent of .kstack/
  } else {
    projectDir = getGitRoot() || process.cwd();
    stateDir = path.join(projectDir, '.kstack');
...
/**
 * Create the .kstack/ state directory if it doesn't exist.
