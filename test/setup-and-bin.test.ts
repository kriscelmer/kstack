...
import { PUBLIC_ROUTED_COMMAND_NAMES } from '../lib/command-registry';
...
describe('setup and bin compatibility', () => {
  test('setup installs into Codex skill directories and uninstall removes it', () => {
    ...
    for (const command of PUBLIC_ROUTED_COMMAND_NAMES) {
      expect(fs.existsSync(path.join(codexHome, 'skills', command))).toBe(false);
    }
    expect(fs.existsSync(path.join(home, '.kstack'))).toBe(true);
    expect(fs.existsSync(path.join(home, '.kstack', 'projects'))).toBe(false);
    ...
    expect(fs.existsSync(path.join(ROOT, '.kstack', 'state', 'main.json'))).toBe(true);
    expect(fs.existsSync(path.join(ROOT, '.kstack', 'contracts', 'main.json'))).toBe(true);
  });
  ...
  test('kstack-config reads and writes only the KStack config location', () => {
